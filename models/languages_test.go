...
func testLanguageToManyPilots(t *testing.T) {
	...
	_, err = tx.Exec("insert into `pilot_languages` (`language_id`, `pilot_id`) values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `pilot_languages` (`language_id`, `pilot_id`) values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	languageDBTypes = map[string]string{`ID`: `int`, `Language`: `text`}
	_               = bytes.MinRead
)
...
func testLanguagesUpsert(t *testing.T) {
	...
	if len(mySQLLanguageUniqueColumns) == 0 {
